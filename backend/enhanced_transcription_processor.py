#!/usr/bin/env python3
"""
Enhanced Transcription Processor - sentiment analysis, audio features,
conversation summarization, and annotation extraction.

This is the consolidated, full-featured processor. It used to live at the
repo root (unreachable - backend/server.py imported a smaller same-named
module that shipped alongside it in backend/) while a stripped-down copy sat
here and was the one actually wired up. They have been merged into this one
file so CONVERSATION_SUMMARIZATION_README.md's documented behaviour
(/transcribe_with_summary, /summarize_transcription, /summary_options) is
reachable.

Heavy/optional dependencies (torch, librosa, vaderSentiment, textblob,
transformers, nltk) are all guarded at import time so this module can still
be imported - and py_compile'd - on a machine that only has numpy installed.
Each guarded feature degrades to a simpler fallback rather than raising.
"""
...
# torch is only ever used here as a type hint (deferred by `from __future__
# import annotations` above, so it need not even be importable) and, in the
# __main__ demo, to build a mock tensor. Production methods only ever call
# duck-typed tensor methods (.squeeze(), .numpy(), .shape, slicing) on
# whatever waveform the caller passes in, so a missing torch never blocks
# real usage - the caller (server.py) already depends on torch directly.
try:
...
class EnhancedTranscriptionProcessor:
    """
    Enhanced transcription processor with sentiment analysis, conversation
    summarization, and annotation capabilities.
    """
    ...
    def __init__(self, sentiment_model: str = "vader"):
        ...
        if not VADER_AVAILABLE:
            logger.warning("vaderSentiment not installed - VADER sentiment scoring disabled")
        ...
        # Advanced text processing (NLTK) is set up lazily on first use.
        # Constructing this class used to import NLTK and, when the corpora
        # were absent, call nltk.download() - an outbound network fetch fired
        # by merely importing/instantiating, in an app that promises no cloud
        # interaction. Nothing here touches the network until a method that
        # actually needs NLTK is called.
        self._nltk_available = None  # None = not probed yet
        self._stopwords = None

    def _ensure_nltk(self) -> bool:
        """Probe for NLTK and its corpora on first use. Returns availability."""
    ...
    def nltk_available(self) -> bool:
        return self._ensure_nltk()

    @property
    def stopwords(self) -> set:
        self._ensure_nltk()
        return self._stopwords if self._stopwords is not None else set()
    ...
    def analyze_sentiment(self, text: str) -> Dict[str, Any]:
        ...
        # VADER Analysis (default engine, but optional - guarded above)
    ...
    def _determine_primary_sentiment(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.sentiment_model == "transformer" and "transformer" in results:
            ...
            # No sentiment engine produced a result (e.g. vaderSentiment not
            # installed and textblob/transformer either unavailable or not
            # selected). Neutral with zero confidence is a safe, honest default.
            return {
                "label": "neutral",
                "confidence": 0.0,
                "scores": {"positive": 0.0, "negative": 0.0, "neutral": 1.0}
    ...
    def _extract_spectral_features(self, audio: np.ndarray, sample_rate: int) -> Dict[str, Any]:
        """Extract spectral features using librosa if available, else a numpy FFT fallback"""
        if sample_rate is None or sample_rate <= 0:
        ...
        if LIBROSA_AVAILABLE:
            try:
                ...
                logger.debug(f"Librosa spectral analysis failed, falling back to FFT: {e}")

        # Fallback to basic spectral analysis (no librosa installed, or it errored)
        try:
    ...
    def analyze_conversation_dynamics(self, segments: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for speaker in speaker_sentiments:
            if speaker_sentiments[speaker]:
                speaker_stats[speaker]["avg_sentiment"] = float(np.mean(speaker_sentiments[speaker]))
            else:
...
def main():
    ...
    if not TORCH_AVAILABLE:
        print("torch is not installed in this environment - skipping the waveform-based demo.")
        print("(This module still works for text-only summarization/annotation without torch.)")
        return
